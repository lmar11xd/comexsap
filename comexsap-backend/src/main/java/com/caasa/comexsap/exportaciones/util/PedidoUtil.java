...
public class PedidoUtil {
	...
	public static String Convertir(String numero, boolean mayusculas) {
		...
		if (Pattern.matches("\\d{1,9},\\d{1,2}", numero)) {
			...
			// parte_decimal = " DÓLARES AMERICANOS CON " + Num[1] + "/100 CENTAVOS.";
			// se convierte el numero a literal
			if (Integer.parseInt(Num[1]) == 0) {// si el valor es cero
				parte_decimal = "DÓLARES AMERICANOS CON CERO CENTAVOS.";
			} else if (Integer.parseInt(Num[1]) > 999999) {// si es millon
				parte_decimal = " DÓLARES AMERICANOS CON " + getMillones(Num[1]) + "CENTAVOS.";
			} else if (Integer.parseInt(Num[1]) > 999) {// si es miles
				parte_decimal = " DÓLARES AMERICANOS CON " + getMiles(Num[1]) + "CENTAVOS.";
			} else if (Integer.parseInt(Num[1]) > 99) {// si es centena
				parte_decimal = " DÓLARES AMERICANOS CON " + getCentenas(Num[1]) + "CENTAVOS.";
			} else if (Integer.parseInt(Num[1]) > 9) {// si es decena
				parte_decimal = " DÓLARES AMERICANOS CON " + getDecenas(Num[1]) + "CENTAVOS.";
			} else {// sino unidades -> 9
				parte_decimal = " DÓLARES AMERICANOS CON " + getUnidades(Num[1]) + "CENTAVOS.";
			}
			// se convierte el numero a literal
