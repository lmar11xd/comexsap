...
import { ExportacionFactura } from '../../documentomaritimo/to/exportacionfactura';
import { DocumentoAereo } from '../to/documentoaereo';
import { DocumentoAereoPosicion } from '../to/documentoaereoposicion';
...
import { EDITAR_DOCUMENTOAEREO, LISTAR_DOCUMENTOAEREO } from 'src/app/shared/breadcrumb/breadcrumb';
import { Utils } from 'src/app/utils/utils';
import { ConfiguracionUtil } from '../../configuracion/configuracion.util';
import { Almacen } from '../../configuracion/to/almacen';
import { Material } from '../../configuracion/to/material';
import { UnidadMedida } from '../../configuracion/to/unidadmedida';
import { PedidoPosicion } from '../../configuracion/to/pedidoposicion';
import { OrdenInterna } from '../../documentomaritimo/to/ordeninterna';
import { InfoCliente } from '../../documentomaritimo/to/infocliente';
import { PedidoFirme } from '../../pedidofirme/to/pedidofirme';
import { MaterialStock } from '../../documentomaritimo/to/materialstock';
...
export class EditarDocumentoAereoComponent implements OnInit {
  ...
  idDespacho = Constantes.P_ID_AEREO;

  idDocumento: string = null;
  dataDocumento: DocumentoAereo;
  dataPosiciones: DocumentoAereoPosicion[] = [];
  dataFactura: ExportacionFactura[] = [];
  posicionesEliminadas: DocumentoAereoPosicion[] = [];
  ...
  constructor(
    ...
    private service: DocumentoAereoService
  ) { }
  ...
  inicializarBreadcrumb(id: string){
    EDITAR_DOCUMENTOAEREO[0].url = EDITAR_DOCUMENTOAEREO[0].url.replace(':id', id);
    let BREADCRUMBS: MenuItem[] = [];
    BREADCRUMBS = BREADCRUMBS.concat(LISTAR_DOCUMENTOAEREO);
    BREADCRUMBS = BREADCRUMBS.concat(EDITAR_DOCUMENTOAEREO);
  ...
  inicializar() {
    ...
    Promise.all([
      ...
      (data :any[]) => {
        ...
        this.dataDocumento = data[10].exportacion;
        this.dataPosiciones = data[10].posiciones;
        this.dataFactura = data[10].facturaImpresion;
        ...
        if(this.dataPosiciones != null) {
          this.dataPosiciones.forEach((posicion: DocumentoAereoPosicion) => {
            ...
            const posicionForm = new FormGroup({
              ...
              pedidoSap: new FormControl(posicion.pedidoSap == '' ? null : posicion.pedidoSap),
  ...
  inicializarFormulario() {
    if(this.dataDocumento != null) {
      ...
      this.documentoForm.controls['numeroContenedor'].patchValue(this.dataDocumento.numeroContenedor ? this.dataDocumento.numeroContenedor : null, {onlySelf: true});
  ...
  irAlListado() {
    ...
    this.router.navigate(['/exportaciones/documentoaereo/listar']);
  }
  ...
  verFacturaComercial() {
    ...
    this.settingsService.mostrarSpinner();
    Promise.all([
      this.service.imprimirFacturaComercial(request, false, true, Constantes.P_FACCOMERCIAL_PESOSAP).toPromise()
    ]).then(
      (data) => {
        this.settingsService.ocultarSpinner();
        const tempBlob = new Blob([data[0]], { type: 'application/pdf' });
    ...
    if( this.tipoDocumento == 0) {
      ...
      Promise.all([
        ...
        (data) => {
          ...
          a.href = url;
          a.download = "EXPORTACION_AEREO_" + this.dataDocumento.codigo + '.pdf';
          a.click();
