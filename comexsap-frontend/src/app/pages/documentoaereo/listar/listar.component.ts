...
import { LISTAR_DOCUMENTOAEREO } from 'src/app/shared/breadcrumb/breadcrumb';
...
export class ListarDocumentoAereoComponent implements OnInit {
  ...
  listado: DocumentoAereo[];
  selectedDocumento: DocumentoAereo;
  ...
  inicializarBreadcrumb(){
    this.breadcrumb2Service.addBreadcrumbs(LISTAR_DOCUMENTOAEREO);
  }

  inicializar() {
    const filtroStg = sessionStorage.getItem(Constantes.P_STG_AEREO_FILTRO);
    if(filtroStg != null) {
  ...
  irACrear() {
    this.router.navigate(['/exportaciones/documentoaereo/crear']);
  }

  irAEditar(documento: DocumentoAereo) {
    this.service.setDocumento(documento);
    this.router.navigate(['/exportaciones/documentoaereo/editar', documento.id]);
  }
  ...
  consultar() {
    sessionStorage.setItem(Constantes.P_STG_AEREO_FILTRO, JSON.stringify(this.filtro));
  ...
  descargar(): void {
    ...
    const name = 'DocumentoAereo_' + Utils.getCurrentTimeId() + '.xlsx';
    let exportData = [];
    this.listado.map(
      row => {
        exportData.push({
    ...
    XLSX.utils.book_append_sheet(book, worksheet, 'Documentos Aereos');

    XLSX.writeFile(book, name);
  }

  mostrarModal(content: string, item: DocumentoAereo) {
