...
import { LISTAR_DOCUMENTOMARITIMO_INTERCOMPANY } from 'src/app/shared/breadcrumb/breadcrumb';
...
export class ListarIntercompanyComponent implements OnInit {
  ...
  inicializarBreadcrumb(){
    this.breadcrumb2Service.addBreadcrumbs(LISTAR_DOCUMENTOMARITIMO_INTERCOMPANY);
  }

  inicializar() {
    const filtroStg = sessionStorage.getItem(Constantes.P_STG_MARITIMO_INTERCOMPANY_FILTRO);
    if(filtroStg != null) {
  ...
  irACrear() {
    this.router.navigate(['/exportaciones/documentomaritimo/intercompany/crear']);
  }

  irAEditar(documento: DocumentoMaritimo) {
    this.service.setDocumento(documento);
    this.router.navigate(['/exportaciones/documentomaritimo/intercompany/editar', documento.id]);
  }
  ...
  consultar() {
    sessionStorage.setItem(Constantes.P_STG_MARITIMO_INTERCOMPANY_FILTRO, JSON.stringify(this.filtro));
