...
import { LISTAR_DOCUMENTOMARITIMO_CARGASUELTA, CREAR_DOCUMENTOMARITIMO_CARGASUELTA } from 'src/app/shared/breadcrumb/breadcrumb';
...
export class CrearCargaSueltaComponent implements OnInit {
  ...
  inicializarBreadcrumb(){
    ...
    BREADCRUMBS = BREADCRUMBS.concat(CREAR_DOCUMENTOMARITIMO_CARGASUELTA);
  ...
  inicializar() {
    ...
    Promise.all([
      ...
      this.configuracionService.listarParametroxCodigoSap(Constantes.P_D012, 'XA2001').toPromise(),//Ruta por defecto
      ...
      (data :any[]) => {
        ...
        this.conversionFamilias = data[11];
        this.etiquetas = data[12];

        if(data[10] != null && data[10].length > 0) {
          this.selectedRuta = Ruta.toObject(data[10][0]);
        }
  ...
  inicializarFormulario() {
    ...
    this.documentoForm.controls['idRuta'].patchValue(this.selectedRuta != null ? this.selectedRuta.id : null, {onlySelf: true});
