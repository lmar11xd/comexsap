...
import { LISTAR_DOCUMENTOMARITIMO_CARGASUELTA } from 'src/app/shared/breadcrumb/breadcrumb';
...
export class ListarCargaSueltaComponent implements OnInit {
  filtro = {
    codigos: [],
    codigosPedidoFirme: [],
    clientes: [],
  ...
  inicializarBreadcrumb(){
    this.breadcrumb2Service.addBreadcrumbs(LISTAR_DOCUMENTOMARITIMO_CARGASUELTA);
  }

  inicializar() {
    const filtroStg = sessionStorage.getItem(Constantes.P_STG_MARITIMO_CARGASUELTA_FILTRO);
    if(filtroStg != null) {
  ...
  irACrear() {
    this.router.navigate(['/exportaciones/documentomaritimo/cargasuelta/crear']);
  }

  irAEditar(documento: DocumentoMaritimo) {
    this.service.setDocumento(documento);
    this.router.navigate(['/exportaciones/documentomaritimo/cargasuelta/editar', documento.id]);
  }
  ...
  consultar() {
    sessionStorage.setItem(Constantes.P_STG_MARITIMO_CARGASUELTA_FILTRO, JSON.stringify(this.filtro));
