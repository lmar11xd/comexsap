...
@Component({
  selector: 'app-modal-pedido-disponible',
  templateUrl: './modal-pedido-disponible.component.html',
  styleUrls: ['./modal-pedido-disponible.component.scss'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      multi: true,
      useExisting: ModalPedidoDisponibleComponent
    }
  ]
})
export class ModalPedidoDisponibleComponent implements OnInit, ControlValueAccessor {
  ...
  buscarPedidos() {
    ...
    Promise.all([
      this.configuracionService.listarPedidoPosicionDisponible({ formulario: this.filtro }).toPromise()
    ]).then(
      (data : any[]) => {
