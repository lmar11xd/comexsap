...
import { LISTAR_PACKINGLIST_AEREO } from 'src/app/shared/breadcrumb/breadcrumb';
...
export class ListarPackingListAereoComponent implements OnInit {
  ...
  inicializarBreadcrumb(){
    this.breadcrumb2Service.addBreadcrumbs(LISTAR_PACKINGLIST_AEREO);
  }

  inicializar() {
    const filtroStg = sessionStorage.getItem(Constantes.P_STG_PACKINGLIST_CARGASUELTA_FILTRO);
    if(filtroStg != null) {
  ...
  irAEditar(packinglist: PackingList) {
    ...
    this.router.navigate(['/exportaciones/packinglist/aereo/editar', packinglist.id]);
  }
  ...
  consultar() {
    sessionStorage.setItem(Constantes.P_STG_PACKINGLIST_CARGASUELTA_FILTRO, JSON.stringify(this.filtro));
