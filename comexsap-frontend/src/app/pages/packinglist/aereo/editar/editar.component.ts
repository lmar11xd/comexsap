...
import { EDITAR_PACKINGLIST_AEREO, LISTAR_PACKINGLIST_AEREO } from 'src/app/shared/breadcrumb/breadcrumb';
...
import { PackingAereo } from '../../to/packing-aereo';
...
export class EditarPackingListAereoComponent implements OnInit {
  ...
  dataPackingList: PackingAereo[] = [];
  packingListEliminadas: PackingAereo[] = [];
  item : number = 0 ;
  ...
  inicializarBreadcrumb(id: string){
    EDITAR_PACKINGLIST_AEREO[0].url = EDITAR_PACKINGLIST_AEREO[0].url.replace(':id', id);
    let BREADCRUMBS: MenuItem[] = [];
    BREADCRUMBS = BREADCRUMBS.concat(LISTAR_PACKINGLIST_AEREO);
    BREADCRUMBS = BREADCRUMBS.concat(EDITAR_PACKINGLIST_AEREO);
  ...
  inicializar() {
    ...
    Promise.all([
      ...
      this.packingListService.obtenerPackingListAereo(this.idExportacion).toPromise()
    ]).then(
      (data :any[]) => {
      ...
      excelData.forEach((row: any) => {
        if(row.CODIGO != null && row.CODIGO != '') {
          ...
          const packingForm = new FormGroup({
            ...
            numero: new FormControl(row.NUMERO),
            codigo: new FormControl(row.CODIGO),
            material: new FormControl(row.MATERIAL),
            cantidad: new FormControl(row.CANTIDAD),
            pesoBruto: new FormControl(pesoBruto),
            dimension: new FormControl(row.DIMENSION)
          });
  ...
  guardarPackingList() {
    ...
    this.settingsService.mostrarSpinner();
    Promise.all([
      this.service.registrarPackingListAereo(request).toPromise()
    ]).then(
      (data : any) => {
        this.settingsService.ocultarSpinner();
  ...
  descargarPackingList(): void {
    const name = 'PackingList_Aereo_' + Utils.getCurrentTimeId() + '.xlsx';
    let exportData = [];
    this.packinglist.controls.forEach((packingForm: FormGroup) => {
      const packingFormData = packingForm.value;
      exportData.push({
  ...
  descargarPdf() {
    ...
    this.service.generarPdf(this.dataDocumento.id, this.versionIngles, this.original).subscribe({
      next: (data) => {
        ...
        a.href = url;
        a.download = "PACKINGLIST_AEREO_" + this.dataDocumento.codigoPacking + '.pdf';
        a.click();
