...
import { REPORTEMARITIMO_CONTENEDORES } from 'src/app/shared/breadcrumb/breadcrumb';
...
export class ContenedoresComponent implements OnInit {
  ...
  inicializarBreadcrumb(){
    this.breadcrumb2Service.addBreadcrumbs(REPORTEMARITIMO_CONTENEDORES);
  }

  inicializar() {
    const filtroStg = sessionStorage.getItem(Constantes.P_STG_REPORTE_CONTENEDORES_FILTRO);
    if(filtroStg != null) {
  ...
  consultar() {
    sessionStorage.setItem(Constantes.P_STG_REPORTE_CONTENEDORES_FILTRO, JSON.stringify(this.filtro));
